...
bistValue ()
{
    ...
    INVAR (NULL == array.values, "initValueArray did not null the code pointer");
    ...
    INVAR (512 == array.capacity, "writeValueArray did not update the capacity");
    INVAR (NULL != array.values, "writeValueArray did not update the code pointer");

    for (int i = 0; i < 256; ++i)
        INVAR (IS_NUMBER (array.values[i]), "writeValueArray did not write the data as a number");
    for (int i = 0; i < 256; ++i)
        INVAR ((123.5 * i) == AS_NUMBER (array.values[i]), "writeValueArray did not write the data value");
    INVAR (IS_NUMBER (array.values[256]), "writeValueArray did not write the data is a number");
    INVAR (1337.0 == AS_NUMBER (array.values[256]), "writeValueArray did not write the data value");
